...
import (
	...
	"github.com/GoogleCloudPlatform/k8s-config-connector/experiments/conductor/cmd/runner/prompts"
...
func BuildMockgcpGcloudScriptCommand(ToolOptions *ToolOptions) *cobra.Command {
	var opts MockgcpGcloudScriptOptions
	opts.ToolOptions = ToolOptions
	cmd := &cobra.Command{
		Use:   "mockgcp-gcloud-script",
		Short: "Create a mockgcp test using gcloud.",
		RunE: func(cmd *cobra.Command, args []string) error {
			out := cmd.OutOrStdout()

			results, err := RunMockgcpGcloudScript(cmd.Context(), opts)
			if err != nil {
				return err
			}
...
// MockgcpGcloudScriptOptions is the options for the MockgcpGcloudScript tool.
type MockgcpGcloudScriptOptions struct {
	*ToolOptions
...
func (o *MockgcpGcloudScriptOptions) DefaultAndValidate() error {
...
// MockgcpGcloudScriptResults holds the results from executing the MockgcpGcloudScript tool.
type MockgcpGcloudScriptResults struct {
...
// RunMockgcpGcloudScript executes the MockgcpGcloudScript tool.
func RunMockgcpGcloudScript(ctx context.Context, opts MockgcpGcloudScriptOptions) (*MockgcpGcloudScriptResults, error) {
	results := &MockgcpGcloudScriptResults{}
	...
	promptOptions := prompts.MockgcpGenerateScriptPromptOptions{
		GcloudCommand: opts.GcloudCommand,
		Group:         opts.Group,
		Resource:      opts.Resource,
	}
	b, err := prompts.GenerateMockgcpGenerateScriptPrompt(promptOptions)
	if err != nil {
		return nil, err
	}
...
func createScriptYaml(ctx context.Context, runnerOptions *RunnerOptions, branch Branch, execResults *ExecResults) ([]string, *ExecResults, error) {
	...
	results, err := RunMockgcpGcloudScript(ctx, opts)
	if err != nil {
		return nil, nil, err
	}
