...
func createScriptYaml(opts *RunnerOptions, branch Branch) {
	...
	stdin.Write([]byte(fmt.Sprintf("git checkout %s && echo done\n", branch.Local)))
	done := false
	...
	stdin.Write([]byte(fmt.Sprintf("git add %s && echo done\n", scriptFile)))
	done = false
	for !done {
		length, err := stdout.Read(outBuffer)
		if err != nil {
	...
	stdin.Write([]byte(fmt.Sprintf("git commit -m \"Adding LLM/gcloud generated test script.yaml for %s\" && echo done\n", branch.Name)))
	done = false
	for !done {
		length, err := stdout.Read(outBuffer)
		if err != nil {
