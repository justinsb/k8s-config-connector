...
func createGithubBranch(opts *RunnerOptions, branch Branch) {
	...
	stdin.Write([]byte(fmt.Sprintf("git branch --list %s && echo done\n", branch.Local)))
	done := false
	...
	if !exists {
		...
		stdin.Write([]byte(fmt.Sprintf("git checkout -b %s && echo done\n", branch.Local)))
		done = false
		for !done {
			length, err := stdout.Read(outBuffer)
			if err != nil {
...
func deleteGithubBranch(opts *RunnerOptions, branch Branch) {
	...
	log.Printf("COMMAND: git checkout -b %s and echo done\r\n", branch.Local)
	stdin.Write([]byte(fmt.Sprintf("git checkout master && echo done\n")))
	done := false
	outBuffer := make([]byte, 1000)
	var msg string
	for !done {
	...
	if exists {
		...
		stdin.Write([]byte(fmt.Sprintf("git branch -D %s && echo done\n", branch.Local)))
		done = false
		for !done {
			length, err := stdout.Read(outBuffer)
			if err != nil {
