...
var _ refsv1beta1.ExternalNormalizer = &MonitoringNotificationChannelRef{}

// MonitoringNotificationChannelRef defines a resource reference to a MonitoringNotificationChannel.
...
// NormalizedExternal provision the "External" value for a resource that depends on MonitoringNotificationChannel.
...
func (r *MonitoringNotificationChannelRef) NormalizedExternal(ctx context.Context, reader client.Reader, otherNamespace string) (string, error) {
	if r.External != "" && r.Name != "" {
		return "", fmt.Errorf("cannot specify both name and external on %s reference", MonitoringNotificationChannelGVK.Kind)
	}
	// From given External
	if r.External != "" {
		var id MonitoringNotificationChannelIdentity
