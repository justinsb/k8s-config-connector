...
var _ refsv1beta1.ExternalNormalizer = &AlertPolicyRef{}

// AlertPolicyRef defines the resource reference to MonitoringAlertPolicy, which "External" field
// holds the GCP identifier for the KRM object.
type AlertPolicyRef struct {
	// A reference to an externally managed MonitoringAlertPolicy resource.
	// Should be in the format "projects/{{projectID}}/locations/{{location}}/alertpolicys/{{alertpolicyID}}".
...
func (r *AlertPolicyRef) NormalizedExternal(ctx context.Context, reader client.Reader, otherNamespace string) (string, error) {
	if r.External != "" && r.Name != "" {
		return "", fmt.Errorf("cannot specify both name and external on %s reference", MonitoringAlertPolicyGVK.Kind)
	}
	// From given External
	if r.External != "" {
		if _, _, err := ParseAlertPolicyExternal(r.External); err != nil {
			return "", err
		}
