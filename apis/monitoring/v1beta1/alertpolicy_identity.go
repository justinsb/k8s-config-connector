...
func (i *AlertPolicyIdentity) String() string {
	return i.parent.String() + "/alertpolicys/" + i.id
}
...
func (i *AlertPolicyIdentity) Parent() *AlertPolicyParent {
	return i.parent
}

type AlertPolicyParent struct {
	ProjectID string
	Location  string
}

func (p *AlertPolicyParent) String() string {
...
func ParseAlertPolicyExternal(external string) (parent *AlertPolicyParent, resourceID string, err error) {
	...
	if len(tokens) != 6 || tokens[0] != "projects" || tokens[2] != "locations" || tokens[4] != "alertpolicys" {
		return nil, "", fmt.Errorf("format of MonitoringAlertPolicy external=%q was not known (use projects/{{projectID}}/locations/{{location}}/alertpolicys/{{alertpolicyID}})", external)
	}
	parent = &AlertPolicyParent{
		ProjectID: tokens[1],
		Location:  tokens[3],
