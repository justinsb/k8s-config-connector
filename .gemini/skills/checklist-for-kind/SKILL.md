...
To track the progress of each resource kind, maintain a journal file in `.gemini/skills/checklist-for-kind/journal/{kind}.md` (using the CamelCase Kind name).
...
2. If not, open a GitHub issue assigned to `codebot-robot` to implement/move the kind to the identity and reference pattern.
