...
If you look at the CRD under `config/crds` (you can use `git diff config/crds`) then you will likely see some changes.  Iterate on the types until they match.
...
You can then run the fuzz-test with `dev/ci/presubmits/test-fuzzers`.  We want the test to pass, but likely it will identify some fields that are not correctly mapped in the fuzz-test.
